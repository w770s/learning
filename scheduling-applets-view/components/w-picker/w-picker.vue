	...
	<view class="w-picker">
		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			<view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
			  ...
			  <view class="w-picker-btn" style="color:#29B6F6" @tap="pickerConfirm">确定</view>
			</view>
			...
			<view class="w-picker-view" v-if="mode=='piece'">
				<picker-view :indicator-style="itemHeight" :value="pickVal" @change="bindChange">
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in data.starts" :key="index">{{item.label}}</view>
					</picker-view-column>
					<picker-view-column>
						<view class="w-picker-item" v-for="(item,index) in data.ends" :key="index">{{item.label}}</view>
					</picker-view-column>
				</picker-view>
			</view>
...
<script>
	...
	import timePiece from './city-data/piece.js';
	...
	export default {
		...
		methods:{
			...
			bindChange(val){
				...
				let year="",month="",day="",hour="",minute="",second="",province,city,area,start,end;
				...
				switch(mode){
					...
					case "region":
						...
						break;
					case "piece":
						start=_this.data.starts[arr[0]].label;
						end=_this.data.ends[arr[1]].label;
						_this.checkArr=[start,end];
						_this.checkValue=[_this.data.starts[arr[0]].value,_this.data.ends[arr[1]].value];
						_this.resultStr=`${start+'-'+end}`;
						break;
					case "selector":
			...
			initData(){
				...
				switch(mode){
					...
					case "time":
						...
						break;
					case "piece":
						start=data.starts[dVal[0]];
						end=data.ends[dVal[1]];
						_this.checkArr=[start.label,end.label];
						_this.checkValue=[start.value,end.value];
						_this.resultStr=`${start.label+'-'+end.label}`;
						break;
					case "region":
