<template>
	<view class="content">
		...
		<!-- 订单待发单状态 -->
		<view class="order-operate" v-if="list.orderStatusValue==='待发单'">
			<view class="order-delete" @tap="deleteBill(list.id)">删除</view>
			<view class="order-edit" @tap="editBill(list.id)">编辑</view>
		</view>
		<!-- 订单跳转详情 -->
		<view class="search-details" 
			v-if="list.orderStatusValue!=='待发单'&&list.orderStatusValue" 
			@tap="orderDetails({id:list.id,status:list.orderStatusValue})">
			查看详情
		</view>
		<!-- 运单跳转详情 -->
		<view class="search-details"
			v-if="(list.waybillStatus==='WAYBILL-STATUS-DQS'&&list.isException===1)
			||(list.waybillStatus!=='WAYBILL-STATUS-DQS'&&list.waybillStatus)" 
		...
		<view class="search-details receivedBy"
			v-if="list.waybillStatus==='WAYBILL-STATUS-DQS'&&list.isException===0" 
			@tap="waybillDetails({id:list.id,status:selectWaybillStatus(list.waybillStatus,list.isException),waybillStatus:list.waybillStatus})">
			签收
		</view>
	</view>
		...
		methods: {
			...
			orderDetails(obj) { // 订单跳转详情
				if(obj.status!=='已拒绝'){
					...
					uni.navigateTo({
						url: `orderNewBill?id=${obj.id}&status=${obj.status}`
					})
				}
			},
