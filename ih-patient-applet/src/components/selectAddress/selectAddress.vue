	...
	export default {
		...
		methods: {
			...
			select_top_item_click(index) {
				...
				if(index==0){
					this.indicatorStyleLeft = 40
				}
				if(index==1){
					this.indicatorStyleLeft = 130
				}
				if(index==2){
					this.indicatorStyleLeft = 215
				}
				if(index==3){
					this.indicatorStyleLeft = 305
				}
			},
			//切换数据列表
			swiperChange(event) {
			...
			address_item_click(swiper_index, index) {
				...
				if (swiper_index < this.pickerSize - 1) {
					...
					setTimeout(() => {
						this.changeIndicator(swiper_index + 1, this.currentObj.name)
					}, 50);
				} else { //是最下一级
				...
				console.log(swiper_index,this.indicatorStyleLeft)
			},
			//返回数据
			returnPicker() {
		...
		mounted() {
			//debugger
			self = this;
			//this.dataList=this.addressDetailArray
		}
	}
</script>
