  ...
  <view class="prompt-box" v-if="visible">
    <view class="prompt">
      ...
      <view class="prompt-buttons" v-if="cancelShow">
        <button class="prompt-cancle"  @click="close">{{closeName}}</button>
        <button class="prompt-confirm" @click="confirm">{{confirmName}}</button>
      </view>
      <view v-if="!cancelShow" style="width: 100%;border-top:1px solid #EAEAEA;">
        <view @click="confirm" style="width: 100%;line-height: 48px;height:48px;color:#4896FF;text-align: center">确定</view>
      </view>
    </view>
  </view>
</template>
