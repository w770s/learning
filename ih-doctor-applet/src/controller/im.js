...
import { updateMultiPortStatus, deepClone, dealMsg, showToast } from '../util/util.js'
 ...
 */
  ...
  onSyncDone() {
    ...
    // wx.navigateTo({
    //   url: `/pages/consulting/consultPage?chatTo=thinking&type=p2p`,
    // })
    // wx.navigateTo({
    //   url: `pages/myNews/index`,
    // })
 ...
 */
  onUpdateSession(session) {
	...
	store.state.user.healthCardId = session.to
    try {
      store.dispatch({
  ...
  */
  ...
  onCustomSysMsg(sysMsg) {
    ...
    console.log(content);
	// console.log(store.state.user.content)
    store.dispatch({
      type: 'Save_sys_content',
      payload: content
    })
    let id = content.id;
    if (id == 1) {
