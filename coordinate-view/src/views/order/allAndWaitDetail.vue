<template>
  <el-card>
    <el-row>
      ...
      <el-col :span="8">
        <el-button icon="el-icon-arrow-left" style="float: right;margin-left: 20px" @click="cancel">返回</el-button>
        <el-button v-if="data.orderHeaderVO.orderStatus==='ORDER-STATUS-DJD'" type="warning" style="float: right;margin-left: 20px" @click="changeOrder('拒绝')">拒绝</el-button>
        <el-button v-if="data.orderHeaderVO.orderStatus==='ORDER-STATUS-DJD'" type="primary" style="float: right;margin-left: 20px" @click="changeOrder('接受')">接受</el-button>
      </el-col>
    </el-row>
    <el-divider />
    <!--装货信息-->
    ...
    changeStatus(data) {
      changeOrderStatus(data).then(res => {
        if (res.code === 1) {
          ...
          this.$router.push('/order/allAndWait')
        } else {
