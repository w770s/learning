    ...
    <el-card>
      <el-row>
        ...
        <el-col :span="8">
          ...
          <el-button v-if="data.waybill.waybillStatus==='WAYBILL-STATUS-DPC'" v-permission="['waybill-wtf-view']" type="danger" class="btn-right" @click="cancelBill">
            取消运单
          </el-button>
          <el-button v-if="data.waybill.waybillStatus!=='WAYBILL-STATUS-DPC'" type="primary" class="btn-right" @click="goodTracking">
            货物追踪
          </el-button>
          <el-button
            v-if="data.waybill.waybillStatus==='WAYBILL-STATUS-DQS'"
            v-permission="['make-sure-receipt']"
            type="primary"
            class="btn-right"
            @click="apiSureReceipt"
          >
            确认回单
          </el-button>
          <el-button
            v-if="rightNowFrim===data.waybill.downstreamEnterpriseName&&data.waybill.waybillStatus==='WAYBILL-STATUS-DPC'&&data.waybill.isException!==1"
          ...
          <el-button
            v-if="rightNowFrim===data.waybill.downstreamEnterpriseName&data.waybill.isTransfer===1&&data.waybill.waybillStatus==='WAYBILL-STATUS-DPC'"
            class="btn-right"
            type="warning"
            @click="cancel(data.waybill.id)"
          >拒绝
          </el-button>
          <el-button
            v-if="(data.waybill.waybillStatus==='WAYBILL-STATUS-DZH'||data.waybill.waybillStatus==='WAYBILL-STATUS-DXH')&&data.waybill.isExceptionEditor!==1&&data.waybill.isException!==1&&data.waybill.isTransfer!==1"
            v-permission="['waybill-cyf-view']"
            class="btn-right"
            type="danger"
            plain
            @click="dialog.abnormalSituation = true"
          >
            异常情况
          </el-button>
          <el-button
            v-if="data.waybill.isException===1&&data.waybill.isExceptionEditor!==1"
            v-permission="['waybill-cyf-view']"
            class="btn-right"
            type="primary"
            @click="overDownButton"
          >
            完成修改
          </el-button>
          <el-button
        ...
        <el-tab-pane label="运单详情" name="waybillDetail">
          ...
          <div>
            ...
            >
              ...
              <el-row class="el-item">
                <el-col :span="8">
                  类型：{{ data.waybill.exceptionShow }}
                </el-col>
                <el-col :span="8">
                  备注 ：{{ data.waybill.remark }}
                </el-col>
              </el-row>
              <el-divider />
            </div>
              ...
              >
                <el-form-item label="车牌号：" prop="vanCert">
                  <el-select v-model="sendCarForms.vanCert" filterable placeholder="请选择">
                    <el-option
                      ...
                      :key="item.plateNo + Math.random(10)"
                      :label="item.plateNo"
                      :value="item.plateNo"
                    />
                  </el-select>
                ...
                <el-form-item label="挂车号：" prop="truckCert">
                  <el-select v-model="sendCarForms.truckCert" filterable placeholder="请选择">
                    <el-option
                      v-for="item in sendCarOption.Vehicles"
                      :key="item.plateNo + Math.random(10)"
                      :label="item.plateNo"
                      :value="item.plateNo"
                    />
                ...
                <el-form-item label="罐体编号：">
                  <el-select v-model="sendCarForms.tankerNo" clearable filterable placeholder="请选择">
                    <el-option
                      ...
                      :key="item.tankerNo + Math.random(10)"
                      :label="item.tankerNo"
                      :value="item.tankerNo"
                    />
                  </el-select>
...
export default {
  ...
  methods: {
    ...
    overDownButton() {
      ...
      const unloadTime = new Date(changeCarOption.reservationUnloadingDate).getTime() - new Date(changeCarOption.reservationLoadingDate).getTime()
      const unloadQuantumStart = parseInt(changeCarOption.unloadingPeriod.split('-')[0].split(':')[0]) - parseInt(changeCarOption.unloadingPeriod.split('-')[1].split(':')[0])
      const unloadQuantumEnd = parseInt(changeCarOption.unloadingPeriod.split('-')[0].split(':')[1]) - parseInt(changeCarOption.unloadingPeriod.split('-')[1].split(':')[1])
      const loadQuantumStart = parseInt(changeCarOption.loadingPeriod.split('-')[0].split(':')[0]) - parseInt(changeCarOption.loadingPeriod.split('-')[1].split(':')[0])
      const loadQuantumEnd = parseInt(changeCarOption.loadingPeriod.split('-')[0].split(':')[1]) - parseInt(changeCarOption.loadingPeriod.split('-')[1].split(':')[1])
      if (loadQuantumStart > 0 || (loadQuantumStart < 0 && loadQuantumEnd > 0)) {
        this.$message.warning('装货时间段结束时间段不能小于开始时间段,请重新选择')
        return
      }
      if (unloadQuantumStart > 0 || (unloadQuantumStart < 0 && unloadQuantumEnd > 0)) {
        this.$message.warning('卸货时间段结束时间段不能小于开始时间段,请重新选择')
        return
      }
      if (unloadTime < 0) {
        this.$message.warning('卸货时间不能小于装货时间,请重新选择日期')
        return
      }
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const obj = {
            ...
            'exceptionShow': this.numberValidateForm.abnormalStatus === '其他（关闭运单)' ? '其他（关闭运单)' : '派车信息填写错误',
            'exceptionStatus': this.numberValidateForm.abnormalStatus === '其他（关闭运单)' ? 'WAYBILL-ERROR-STATUS-OTHER' : 'WAYBILL-ERROR-STATUS-INFO',
    ...
    apiSendCar() {
      ...
      const truckCertData = this.sendCarOption.Vehicles.find(res => {
      ...
      const unloadTime = new Date(this.sendCarForms.reservationUnloadingDate).getTime() - new Date(this.sendCarForms.reservationLoadingDate).getTime()
      if (unloadTime < 0) {
        this.$message.warning('卸货时间不能小于装货时间,请重新选择日期')
        return
      }
    ...
    apiGetVehicleList() {
      const params = {
        pageNum: 1,
        size: 1000,
        type: 'SEMI_TRAILER,FULL_TRAILER,VEHICLE_TRAILER'
      }
      getTrailerList(params).then(res => {
        if (res.code === 1) {
          this.sendCarOption.Vehicles = res.data.list
        } else {
          this.$message.error('获取车辆失败')
        }
      }).catch(err => {
    ...
    cancelBill() {
      const that = this
      this.$confirm('你正要取消运单, 是否继续?', '取消运单', {
        ...
        const param = {
          ...
          waybillStatus: 'WAYBILL-STATUS-YGB'
        }
        updateWaybill(param).then(res => {
          if (res.code === 1) {
            that.$message.success('取消运单成功')
            that.apiGetOrderDetail()
          } else {
            that.$message.error(res.code.msg)
          }
        }).catch(err => {
          that.$message.error('取消运单失败')
          console.log(err)
        })
      })
    },
